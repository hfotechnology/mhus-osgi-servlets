...
public class Client {
    ...
    // Sample:
    // jws|local|http://localhost:8181/cxf/hehe?wsdl|http://impl.ws_server.ws.test.mhus.de/|WSServiceImplService

    protected Target createTarget(String url) throws IOException {

        String[] parts = url.split("\\|");
        TargetFactory factory = getFactory(parts[0]);
        if (factory == null) throw new FactoryNotFoundException(parts[0], url);
