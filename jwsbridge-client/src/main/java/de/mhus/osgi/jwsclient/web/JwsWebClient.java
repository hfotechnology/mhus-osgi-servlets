...
public class JwsWebClient extends Client {
    ...
    public static synchronized JwsWebClient instance() {
        if (instance == null) instance = new JwsWebClient();
        return instance;
    }
    ...
    public Connection getConnection(Object session, String target) throws Exception {
        ...
        if (session == null) return getTarget(target).createConnection();

        synchronized (session) {
            Connection con =
                    (Connection)
                            getSessionAttribute(session, SESSION_CONNECTION_ATTRIBUTE + target);
            if (con == null) {
    ...
    public static void setSessionAttribute(Object session, String name, Object object)
            throws Exception {
